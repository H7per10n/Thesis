...
| Memory | 128 MB DDR3L |
| OS | Linux (Buildroot) |
| CAN transceiver | SN65HVD230 (3.3 V, up to 1 Mbit/s) |
...
| Parameter | Value |
|---|---|
| MCU | Teensy 4.0 (NXP i.MX RT1062, ARM Cortex-M7, 600 MHz) |
| Gate driver | Texas Instruments DRV8302 (SLVSA73C) |
...
| Pole pairs | 7 |
| Phase resistance | Measured at runtime via characteriseMotor() |
| Phase inductance | Measured at runtime via characteriseMotor() |
...
| Transceiver (all nodes) | Texas Instruments SN65HVD230 |
| Protocol | CANCommander register-based (SimpleFOC community) |
...
|---|---|---|---|
| Supervisor | Generated C99 from CIF/ESCET | 100 Hz (10 ms) | System-wide coordination, safety logic, fault recovery |
| Resource control (FOC) | SimpleFOC library | ~20 kHz | Phase current regulation, PWM generation, motor commutation |
...
| 19-16 | Packet type | 0x1 read, 0x2 write, 0x3 response, 0x4 event |
| 15-8 | Register / event number | 0x00-0xFF |
| 7-0 | Motor index | 0x00-0xFF |

### Custom CAN Registers (supervisor interface)

| Address | Name | CIF event | Direction |
|---|---|---|---|
| 0xF0 | State | (read-only) | Node to supervisor |
| 0xF1 | Calibrate | c_calibrate | Supervisor to node |
| 0xF2 | Enable | c_enable | Supervisor to node |
| 0xF3 | Stop | c_stop | Supervisor to node |
| 0xF4 | Recover | c_recover | Supervisor to node |
| 0xF5 | Cal. reject | c_cal_reject | Supervisor to node |
| 0xF6 | Velocity | c_set_velocity | Supervisor to node |
| 0xF8 | Reboot | c_reboot | Supervisor to node |
...
**Node firmware**
- Arduino or PlatformIO toolchain for Teensy 4.0
- SimpleFOC library (v2.x)
- FlexCAN_T4 library
...
This produces `bin/NET_engine_arm`. Deploy to the Luckfox board:
...
Open `Arduino/TEMPO.ino` in PlatformIO or the Arduino IDE configured for Teensy 4.0. Install the SimpleFOC and FlexCAN_T4 libraries. Upload to each node.
...
- Texas Instruments. DRV8302 Three Phase Gate Driver With Dual Current Shunt Amplifiers and Buck Regulator. SLVSA73C, Rev. C (2016).
- Texas Instruments. SN65HVD230 3.3-V CAN Bus Transceiver. SLLS559H (2001, revised 2018).
- Rockchip Electronics. RK3506G2 Datasheet, Rev. 1.3 (2025).
...
This repository contains the source material for an integrated master's thesis. The CIF models, generated code, and integration code are provided for academic and engineering reference. The Eclipse ESCET toolkit is available under the Eclipse Public License. SimpleFOC is licensed under the MIT License.
